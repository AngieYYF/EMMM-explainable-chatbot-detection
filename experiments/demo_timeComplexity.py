...
def main():
    ...
    if args.dataset == 'mwoz': 
        ...
        slot_info = pd.read_json("output_template/Datasets/ontology_mwoz_new.json").reset_index()
    elif args.dataset == 'frames': 
        sensitive_info_masker = Info_Masker(frames_info_masking_prompt)
        da_extractor = Frames_DA_Extractor()
        slot_info = pd.read_json("output_template/Datasets/ontology_frames_new.json").reset_index()
   
    DEFAULT_DEMO_ID = 1000
    ...
    demo_runner = EMMM_Demo(framework, sensitive_info_masker, da_extractor, report_generator, DEFAULT_DEMO_ID, log=False)

    # randomly sample 100 utterances from the test set
    usr_utts = test_dataset.apply(
        lambda x: [(x['dia_no'], utt) for utt in x['dia'].split("\n") if utt.startswith("user: ")], 
        axis=1
    )
    usr_utts = [(utt[0], turn_no, utt[1]) for sublist in usr_utts for turn_no, utt in enumerate(sublist)]
    sample_size = min(100, len(usr_utts))
    random.seed(RANDOM_STATE)
    usr_utts = random.sample(usr_utts, sample_size)

    time_complexity_results = []
    for dia_no, turn_no, utt in tqdm(usr_utts): 
        _, sample_time_complexity = demo_runner.detect_explain_utt("system: DEMO DOES NOT AFFECT ANYTHING.", utt)
        time_complexity_results.append({'dia_no': dia_no, 'turn_no': turn_no, 'utt': utt, 'time_complexity': sample_time_complexity})
        demo_runner.reset_demo(DEFAULT_DEMO_ID)

    # Compute mean and std for each time metric
    time_metrics = defaultdict(list)
    for result in time_complexity_results:
        time_complexity = result['time_complexity']
        for key, value in time_complexity.items():
    ...
    for key, stats in aggregated_stats.items():
        print(f"{key}: avg = {stats['avg']:.4f}, std = {stats['std']:.4f}", flush=True)


    # Save the time complexity results
    output_data = {
        "aggregated_stats": aggregated_stats, 
        "results": time_complexity_results  
    }
    save_dir = os.path.join(args.save_dir, 'demo', 'time_complexity')
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, "time_complexity_summary.json")
    with open(save_path, "w") as f:
        json.dump(output_data, f, indent=2)

    print(f"Saved time complexity results to: {save_path}", flush=True)
...
# python3 experiments/demo_timeComplexity.py \
#     -da_extraction_method "qwen" \
#     -dataset "frames" \
#     -use_da_value "False" \
#     -use_da_turn "False" \
