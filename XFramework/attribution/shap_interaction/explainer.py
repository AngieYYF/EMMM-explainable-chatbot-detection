...
class SamplePerturbation():
    ...
    def sample_perturbations(self, n_perturbation, n_features, instance,
        ...
        Xs_aug = self.data_mapping_function(Is_aug, instance) # for text data, remove words with 0 values, and return words with 1 values in Is_aug
    ...
    def generate_Is(self, n_perturbation, n_features, method = 'banzhaf',
        ...
        if method == 'banzhaf_all' or n_perturbation == math.pow(2,n_features):
            ...
            for i in range(1,l): # for each order < l
                for inter in list(combinations(range(d), i)): # different selection of i features from d features
                    x = np.zeros(d)
                    x[np.array(inter)] = 1 
                    Is.append(x)
                    cnt += 1
            total = cnt + int(scipy.special.comb(d, l)) # scipy.special.comb(d, l) = #combinations in #features choose l

            # each lower-order combination requires a perturbation, rest is distributed across all combinations, round-up
            n_permutation = int((n_perturbation - cnt) / total) + 1
            #print("Number of permutation:", n_permutation)
            for _ in range(n_permutation):
                p = np.random.permutation(range(d)) # permute of all feature (random order them)
                for inter in list(combinations(range(d), l)): # get all combinations of order l, p[combo] = actual feature combo of order l
                    key = tuple(sorted([ p[x] for x in inter])) 
                    # get a permutation of all features
                    # each feature combo have features at index iter in permutation
                    # for each feature combo, a sampled context = all features in the permutation preceding all selected features
                    # selected features with earliest index in the permutation has index inter[0], so context = p[:inter[0]]
                    base = p[:inter[0]] # context
                    for T in powerset(set(key)): # for each context, the estimation of high-order partial derivative requires inference for all powerset of the selected features
                        x = np.zeros(d)
                        x[base] = 1
                        if len(T) > 0:
...
class TextExplainer(Explainer):
    ...
    def explain_instance(self, instance, train_perturbation,
                         valid_perturbation = None, feature_split = None):
        '''
        Args:
        ...
        # group the feature_combo text with their values
        names = []
        if feature_split is None: 
            instance = instance.split()
        else: 
            instance = feature_split(instance)
        ...
        explaination = sorted(names, key = lambda x:abs(x[1]), reverse = True) # sort to have highest score first
